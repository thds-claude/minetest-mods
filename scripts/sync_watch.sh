...
commit_log() {
    ...
    git add logs/
    git commit -m "log($(hostname -s)): $(STAMP)" \
        --author="sync-bot <sync@$(hostname -s)>" >> "$LOG_FILE" 2>&1 || true
    git push origin "$BRANCH" >> "$LOG_FILE" 2>&1 || true
}

log "=== sync_watch gestartet: $(hostname -s) ==="
last_commit=""
while true; do
    cd "$REPO_DIR"
    ...
    if [[ "$remote_commit" != "$last_commit" ]]; then
        ...
        sync_files; restart_server; commit_log
